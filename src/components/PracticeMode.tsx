  ...
  const generateDecompositionSteps = (a: number, b: number): string[] => {
    if (a > 10) {
      ...
      return [
        ...
        t.decompositionStep2?.replace('{a}', a.toString()).replace('{b}', b.toString()).replace('{tens}', tens.toString()).replace('{ones}', ones.toString()) || `${a} × ${b} = (${tens} + ${ones}) × ${b}`,
  ...
  const generateSmartExplanation = useCallback((a: number, b: number, attempts: number): ExplanationContent => {
    ...
    if (a === 9 || b === 9) {
      ...
      return {
        ...
        steps: [
          ...
          t.ninesStep3?.replace('{other}', other.toString()).replace('{calc1}', (10 * other).toString()).replace('{calc2}', (9 * other).toString()) || `Then subtract ${other}: ${10 * other} - ${other} = ${9 * other}`,
    ...
    if (a === 11 && b < 10) {
      return {
        ...
        steps: [
          ...
          t.elevensStep2?.replace('{digit}', b.toString()) || `Just write the digit twice: ${b}${b}`,
    ...
    if (userProgress.strugglingWith.includes(a) || userProgress.strugglingWith.includes(b) || attempts > 1) {
      return {
        ...
        concept: t.visualArrayConcept?.replace('{a}', a.toString()).replace('{b}', b.toString()) || `Think of ${a} × ${b} as making ${b} groups of ${a} objects`,
