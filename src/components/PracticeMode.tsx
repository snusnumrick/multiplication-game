...
import {ArrowLeft, Brain, Ear, Eye, Hand, Target, Zap} from 'lucide-react';
...
export function PracticeMode() {
    ...
    const checkAnswer = useCallback(() => {
        ...
        if (isRight) {
            ...
            // Auto-select the input text for easy correction
            setTimeout(() => {
                const input = document.querySelector('input[type="number"]') as HTMLInputElement;
                if (input) {
                    input.focus();
                    input.select(); // This selects all text
                }
            }, 100);
    ...
    if (!isMounted) {
        return (
            <div
                className="min-h-screen bg-gradient-to-br from-green-200 via-blue-200 to-purple-300 flex items-center justify-center">
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-green-200 via-blue-200 to-purple-300 p-4">
            {/* Header */}
            <div className="flex items-center justify-between mb-8 pt-4 mobile-compact">
                <button
                    onClick={() => {
                        playSound?.('click');
            ...
            {selectedTable && settings.legendEnabled && (
                <div
                    className="fixed bottom-4 left-4 bg-white/95 backdrop-blur-sm rounded-2xl p-3 shadow-xl max-w-xs transition-all duration-300">
                    <h4 className="font-bold text-gray-700 mb-1 text-sm">{t.smartStrategiesTitle}</h4>
                    <div className="space-y-1 text-xs">
                        <div
                            className={`flex items-center py-1 px-2 rounded-lg transition-all duration-300 ${explanation && getStrategyCategory(explanation.strategy) === 'visual' ? 'bg-green-100 scale-105' : 'bg-transparent'}`}>
                            <Eye className="w-4 h-4 text-green-500 mr-2"/>
                            <span
                                className={`${explanation && getStrategyCategory(explanation.strategy) === 'visual' ? 'font-bold text-green-800' : 'text-gray-600'}`}>{t.visualStrategyLabel}</span>
                        </div>
                        <div
                            className={`flex items-center py-1 px-2 rounded-lg transition-all duration-300 ${explanation && getStrategyCategory(explanation.strategy) === 'pattern' ? 'bg-purple-100 scale-105' : 'bg-transparent'}`}>
                            <Zap className="w-4 h-4 text-purple-500 mr-2"/>
                            <span
                                className={`${explanation && getStrategyCategory(explanation.strategy) === 'pattern' ? 'font-bold text-purple-800' : 'text-gray-600'}`}>{t.patternStrategyLabel}</span>
                        </div>
                        <div
                            className={`flex items-center py-1 px-2 rounded-lg transition-all duration-300 ${explanation && getStrategyCategory(explanation.strategy) === 'counting' ? 'bg-blue-100 scale-105' : 'bg-transparent'}`}>
                            <Target className="w-4 h-4 text-blue-500 mr-2"/>
                            <span
                                className={`${explanation && getStrategyCategory(explanation.strategy) === 'counting' ? 'font-bold text-blue-800' : 'text-gray-600'}`}>{t.countingStrategyLabel}</span>
                        </div>
                        <div
                            className={`flex items-center py-1 px-2 rounded-lg transition-all duration-300 ${explanation && getStrategyCategory(explanation.strategy) === 'breakdown' ? 'bg-orange-100 scale-105' : 'bg-transparent'}`}>
                            <Brain className="w-4 h-4 text-orange-500 mr-2"/>
                            <span
                                className={`${explanation && getStrategyCategory(explanation.strategy) === 'breakdown' ? 'font-bold text-orange-800' : 'text-gray-600'}`}>{t.breakdownStrategyLabel}</span>
                        </div>
                        <div
                            className={`flex items-center py-1 px-2 rounded-lg transition-all duration-300 ${explanation && getStrategyCategory(explanation.strategy) === 'kinesthetic' ? 'bg-red-100 scale-105' : 'bg-transparent'}`}>
                            <Hand className="w-4 h-4 text-red-500 mr-2"/>
                            <span
                                className={`${explanation && getStrategyCategory(explanation.strategy) === 'kinesthetic' ? 'font-bold text-red-800' : 'text-gray-600'}`}>{t.kinestheticStrategyLabel}</span>
                        </div>
                        <div
                            className={`flex items-center py-1 px-2 rounded-lg transition-all duration-300 ${explanation && getStrategyCategory(explanation.strategy) === 'auditory' ? 'bg-yellow-100 scale-105' : 'bg-transparent'}`}>
                            <Ear className="w-4 h-4 text-yellow-500 mr-2"/>
                            <span
                                className={`${explanation && getStrategyCategory(explanation.strategy) === 'auditory' ? 'font-bold text-yellow-800' : 'text-gray-600'}`}>{t.auditoryStrategyLabel}</span>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="fixed top-16 right-16 animate-spin-slow">
                <img src="/images/math-symbols.jpg" alt={t.mathSymbolsAlt}
